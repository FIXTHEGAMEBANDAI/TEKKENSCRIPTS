...
local myScript = sdk2.Register:New("Tekken PANDA","\xef\x93\x9a")
local outModule = {}
		...
		if isCounterAttackPossible(FrameAdvantage) then
			...
			if AttackType == enums.AttackType.MID then
				...
				if FrameAdvantage >= 20 then
					executeMacro(enums.Input.FORWARD+ enums.Input.DOWN, enums.Input.R_HAND, 5, enums.InputType.KEY_DOWN)
				...
				if FrameAdvantage == 19 then
					...
					executeMacro(0 , enums.Input.L_HAND +  enums.Input.R_HAND, 2, enums.InputType.KEY_PRESS)
					return
				end
				--OK
				if FrameAdvantage == 18 then
					executeMacro(enums.Input.DOWN + enums.Input.FORWARD , 0, 1, enums.InputType.KEY_PRESS)
					executeMacro(enums.Input.DOWN + enums.Input.FORWARD , enums.Input.R_HAND, 1, enums.InputType.KEY_PRESS)
					executeMacro(0 , enums.Input.L_HAND, 2, enums.InputType.KEY_PRESS)
					return
				end
				--OK
				...
				--OK
				if FrameAdvantage == 15 or FrameAdvantage == 14 or FrameAdvantage == 13 or FrameAdvantage == 12 then
...
myScript:onMenu(function()
    ...
    menu.text('PANDA Combo Test Script Demo')
    menu.sameLine()
