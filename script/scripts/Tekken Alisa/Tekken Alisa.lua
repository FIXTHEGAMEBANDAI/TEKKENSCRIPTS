...
local myScript = sdk2.Register:New("Tekken Alisa","\xef\x82\x84")
local outModule = {}
	...
	if self.Distance < 2000.00 then
		...
		if isCounterAttackPossible(FrameAdvantage) then
			...
			if AttackType == enums.AttackType.MID then
				--OK
				if FrameAdvantage >= 21 then
					executeMacro(enums.Input.BACK, enums.Input.L_KICK + enums.Input.R_KICK, 5, enums.InputType.KEY_PRESS)
					return
				end
				--OK
				...
				--OK
				if FrameAdvantage == 15 or FrameAdvantage == 16 then
				...
				if FrameAdvantage == 13 then
					executeMacro(enums.Input.DOWN + enums.Input.FORWARD, enums.Input.R_KICK, 5, enums.InputType.KEY_PRESS)
					return
				end
				--OK
			...
			if AttackType == enums.AttackType.LOW then
				...
				if FrameAdvantage == 18 then
					executeMacro(enums.Input.UP, enums.Input.R_HAND, 10, enums.InputType.KEY_PRESS)
					return
				end
				if FrameAdvantage == 17 then
					executeMacro(0, enums.Input.L_HAND, 5, enums.InputType.KEY_PRESS)
				...
				--OK
				if FrameAdvantage == 13 then
					executeMacro(0, enums.Input.L_HAND, 5, enums.InputType.KEY_DOWN)
...
myScript:onMenu(function()
    ...
    menu.text('Alisa Combo Script Core')
    menu.sameLine()
