...
local myScript = sdk2.Register:New("Tekken JACK-8","\xef\x92\x90")
local outModule = {}
	...
	if self.Distance < 2000.00 then
		...
		if isCounterAttackPossible(FrameAdvantage) then
			...
			if AttackType == enums.AttackType.MID then
				...
				--OK
				if FrameAdvantage == 14 or FrameAdvantage == 12 or FrameAdvantage == 13 then
...
myScript:onMenu(function()
    ...
    menu.text('JACK-8 Combo Test Script Demo')
    menu.sameLine()
