...
local myScript = sdk2.Register:New("Tekken KUMA","\xef\x93\x9a")
local outModule = {}
	...
	if self.Distance < 2000.00 then
		...
		if isCounterAttackPossible(FrameAdvantage) then
			...
			if AttackType == enums.AttackType.HIGH then
				--OK
				if FrameAdvantage == 12 or FrameAdvantage == 11 or FrameAdvantage == 10 then
			...
			if AttackType == enums.AttackType.MID then
				...
				if FrameAdvantage >= 18 then
					...
					executeMacro(enums.Input.BACK + enums.Input.DOWN, enums.Input.L_HAND, 5, enums.InputType.KEY_PRESS)
			...
			if AttackType == enums.AttackType.LOW then
				...
				--OK
				if FrameAdvantage == 15 or FrameAdvantage == 14 or FrameAdvantage == 13 then
...
myScript:onMenu(function()
    ...
    menu.text('KUMA Combo Test Script Demo')
    menu.sameLine()
    ...
    menu.text("It allows for automatic blocKUMA and punishments with customizable chances.")
