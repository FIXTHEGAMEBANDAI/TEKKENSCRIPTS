...
local myScript = sdk2.Register:New("Tekken JUN","\xef\x93\x9a")
local outModule = {}
		...
		if isCounterAttackPossible(FrameAdvantage) then
			...
			if AttackType == enums.AttackType.MID then
				...
				if FrameAdvantage >= 21 then
					executeMacro(enums.Input.UP+enums.Input.FORWARD, enums.Input.R_HAND, 5, enums.InputType.KEY_PRESS)
					return
				end
				--OK
				...
				if FrameAdvantage == 14 then
					executeMacro(enums.Input.BACK, enums.Input.R_KICK, 5, enums.InputType.KEY_DOWN)
					executeMacro(0, enums.Input.R_HAND, 5, enums.InputType.KEY_PRESS)
					return
				end
				--OK
...
myScript:onMenu(function()
    ...
    menu.text('JUN Combo Test Script Demo')
    menu.sameLine()
