...
local myScript = sdk2.Register:New("Tekken PAUL","\xef\x93\x9a")
local outModule = {}
		...
		if isCounterAttackPossible(FrameAdvantage) then
			...
			if AttackType == enums.AttackType.MID then
				...
				if FrameAdvantage >= 18 then
					executeMacro(enums.Input.UP + enums.Input.FORWARD , 0, 1, enums.InputType.KEY_PRESS)
					executeMacro(enums.Input.UP + enums.Input.FORWARD , enums.Input.L_KICK, 5, enums.InputType.KEY_PRESS)
					executeMacro(0 , enums.Input.R_KICK, 2, enums.InputType.KEY_PRESS)
					return
				end
				--OK
				if FrameAdvantage == 17 or FrameAdvantage == 16 then
					executeMacro(enums.Input.FORWARD + enums.Input.UP, enums.Input.L_KICK, 5, enums.InputType.KEY_PRESS)
...
myScript:onMenu(function()
    ...
    menu.text('PAUL Combo Test Script Demo')
    menu.sameLine()
