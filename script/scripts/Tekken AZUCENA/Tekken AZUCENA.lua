...
local myScript = sdk2.Register:New("Tekken AZUCENA","\xee\x81\x99")
local outModule = {}
		...
		if isCounterAttackPossible(FrameAdvantage) then
			...
			if AttackType == enums.AttackType.MID then
				...
				if FrameAdvantage == 13 then
					...
					executeMacro(0, enums.Input.R_KICK, 4, enums.InputType.KEY_PRESS)
					return
				end
				--OK
				if FrameAdvantage == 12 then
			...
			--OK
				if FrameAdvantage == 13 then
					executeMacro(0, enums.Input.R_KICK, 5, enums.InputType.KEY_DOWN)
...
myScript:onMenu(function()
    ...
    menu.text('AZUCENA Combo Script Core')
    menu.sameLine()
