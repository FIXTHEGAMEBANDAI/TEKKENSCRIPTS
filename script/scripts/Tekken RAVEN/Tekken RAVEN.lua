...
local myScript = sdk2.Register:New("Tekken RAVEN","\xef\x93\x9a")
local outModule = {}
		...
		if isCounterAttackPossible(FrameAdvantage) then
			...
			if AttackType == enums.AttackType.MID then
				...
				if FrameAdvantage == 15 then
					executeMacro(enums.Input.FORWARD, enums.Input.R_KICK, 5, enums.InputType.KEY_PRESS)
				end
				--OK
...
myScript:onMenu(function()
    ...
    menu.text('RAVEN Combo Test Script Demo')
    menu.sameLine()
