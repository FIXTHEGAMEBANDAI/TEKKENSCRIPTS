...
local myScript = sdk2.Register:New("Tekken DEVILDJIN","\xef\x94\x95")
local outModule = {}
		...
		FrameAdvantage = getLocalFrameAdvantage()
			...
			if AttackType == enums.AttackType.MID then
				...
				if FrameAdvantage == 24 then
					executeMacro(enums.Input.FORWARD, 0, 3, enums.InputType.KEY_DOWN)
					executeMacro(enums.Input.FORWARD +enums.Input.DOWN, 0, 5, enums.InputType.KEY_DOWN)
			...
			--OK
				if FrameAdvantage == 13 then
					executeMacro(0, enums.Input.L_HAND, 8, enums.InputType.KEY_PRESS)
					executeMacro(0, enums.Input.R_HAND, 5, enums.InputType.KEY_PRESS)
					return
				end
			--OK
			...
			if AttackType == enums.AttackType.PROJ then
