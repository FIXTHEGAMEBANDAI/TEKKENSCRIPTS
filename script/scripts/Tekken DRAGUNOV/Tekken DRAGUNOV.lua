...
local myScript = sdk2.Register:New("Tekken DRAGUNOV","\xef\x91\xb0")
local outModule = {}
		...
		if isCounterAttackPossible(FrameAdvantage) then
			...
			if AttackType == enums.AttackType.MID then
				...
				if FrameAdvantage == 14 then
					executeMacro(enums.Input.BACK, enums.Input.R_KICK, 5, enums.InputType.KEY_PRESS)
			...
			if AttackType == enums.AttackType.LOW then
				...
				--OK
				if FrameAdvantage == 18 or FrameAdvantage == 17 then
					executeMacro(0, enums.Input.R_HAND, 10, enums.InputType.KEY_PRESS)
					return
				end
				--OK
				if FrameAdvantage == 16 or FrameAdvantage == 15 or FrameAdvantage == 14 or FrameAdvantage == 13 or FrameAdvantage == 12 then
...
myScript:onMenu(function()
    ...
    menu.text('DRAGUNOV Combo Test Script Demo')
    menu.sameLine()
