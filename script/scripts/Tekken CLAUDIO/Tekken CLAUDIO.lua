...
local myScript = sdk2.Register:New("Tekken CLAUDIO","\xef\x92\xbe")
local outModule = {}
		...
		FrameAdvantage = getLocalFrameAdvantage()
			...
			if AttackType == enums.AttackType.LOW then
				...
				if FrameAdvantage == 16 or FrameAdvantage == 15 then
					executeMacro(enums.Input.UP + enums.Input.FORWARD, enums.Input.R_HAND, 6, enums.InputType.KEY_DOWN)
					executeMacro(0, enums.Input.R_HAND, 5, enums.InputType.KEY_PRESS)
					return
				end
				--OK
				if FrameAdvantage == 14 then
					executeMacro(enums.Input.DOWN + enums.Input.FORWARD, enums.Input.R_HAND, 6, enums.InputType.KEY_DOWN)
...
myScript:onMenu(function()
    ...
    menu.text('CLAUDIO Combo Script Core')
    menu.sameLine()
